...
export const getEnglishTitleFromIntArrays = (
  ...
  const englishTitle = title.find((item) => item._key === 'en')?.value || fallbackMessage
  return englishTitle
}
...
export const getUkrainianTitleFromIntArrays = (
  ...
  const uaTitle = title.find((item) => item._key === 'ua')?.value || fallbackMessage
  return uaTitle
}
...
export const getUkrainianBlockFromIntArrays = (
  ...
  const uaBlocks = blocks.find((item) => item._key === 'ua')?.value || fallbackMessage
  return uaBlocks
}
