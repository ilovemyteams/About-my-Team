    ...
    },
    {
      ...
      options: {
        source: (_, options) => {
          ...
          if (parent?.subtitle) {
            const enSubtitle = parent.subtitle.find((item) => item._key === 'en')?.value
            return enSubtitle
          }
        },
        maxLength: 50,
      ...
      validation: (Rule) =>
        Rule.custom((value, context) => {
          ...
          const currentValue = (value as {current?: string})?.current

          if (parent?.subtitle) {
            const enSubtitle = parent.subtitle.find((item) => item._key === 'en')?.value || ''

            const expectedSlug = enSubtitle.toLowerCase().replace(/\s+/g, '-').slice(0, 50)
